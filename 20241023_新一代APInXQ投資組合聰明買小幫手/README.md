...
功能涵蓋如下:
* py_exe<br>
  本資料夾底下涵蓋編譯好的教學範例執行檔和執行需要的資源，可以直接開啟**Python投資組合聰明買小幫手(教學範例).exe**做使用
* buy_smart_portfolio.py<br>
  **本檔案為主要程式也是執行檔原碼**，GUI套件使用Pyside6，SDK版本為2.0.1，Python版本可為3.8~3.12，有任何需求都可以從這邊做修正，直接執行本程式碼即可看到GUI畫面
  * 此版本可支援零股買賣，但參考價的等待都以整股報價為準，到價送出漲停買的限價委託
  * 預設支援最後等待時間為13:20，時間到不論價格如何都會送出委託，最後等待時間可自行調整
* buy_smart_portfolio.spec<br>
...
* Step 2. 對讀取之目標清單及當前庫存做下單試算
* Step 3. 試算完確認無誤後即可點擊"開始下單"，程式會自動開始監控是否低於參考價，及是否觸及最後等待時間
